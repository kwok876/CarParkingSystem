...
CarPark::CarPark(int id, string name, string location, int* slot, double* fee) {
	...
	_slot = new CarParkSlot[total];
	for (int i = 0; i < slot[0]; i++) {
		_slot[i] = CarParkSlot(i + 1, id, 0);
	}
	for (int i = slot[0]; i < slot[1]; i++) {
		_slot[i] = CarParkSlot(i + 1, id, 1);
	}
	for (int i = slot[1]; i < slot[2]; i++) {
		_slot[i] = CarParkSlot(i + 1, id, 2);
	}
	for (int i = 0; i < 3; i++) {
		_fee[i] = fee[i];
	}
...
CarParkSlot CarPark::getSlot(int i) { return _slot[i]; }
CarParkSlot CarPark::setSlot(int i){}